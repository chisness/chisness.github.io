...
Specifically, we set the strategy for the best action $$P(a_{b}) = 1 - \epsilon + \frac{\epsilon}{num_actions}$$. All other actions are $$P(a_{o})= \frac{\epsilon}{\num_actions}$$. Where $$\epsilon$$ is a small value. If $$\epsilon = 0.1$$ and we had 2 actions, we would play the best action 95% of the time and the other action 5% of the time. This is called soft greedy; greedy would be playing the best estimated action 100% of the time. 
